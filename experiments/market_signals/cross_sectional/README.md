...
```bash
# 생존편향(survivorship) 민감도 시뮬 — 30년 패널 필요
python -m experiments.market_signals.cross_sectional.run_survivorship            # base 시나리오
CS_SURV_K=6 CS_SURV_TLO=-1.0 CS_SURV_THI=-1.0 CS_SURV_TAG=harsh \
  python -m experiments.market_signals.cross_sectional.run_survivorship          # harsh 시나리오
```

**생존편향 시뮬(`survivorship.py`, `run_survivorship.py`)**은 폐지 종목 데이터를 구할 수 없어
(yfinance는 Lehman·Enron 등 폐지 종목을 삭제 → 0 rows) **편향의 크기를 추정**한다. 취약종목(고변동·
저유동)에 상장폐지+손실을 주입해 vol/beta·소형 프리미엄이 살아남는지 본다. 결론: 둘 다 신뢰 불가(§8).
