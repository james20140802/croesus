...
# 자동 갱신 한 회의 기본 시간 예산(초). 저하된 로컬 LLM이 shortlist를 종목마다
# 수백 초씩 붙들어 DuckDB 쓰기 락을 몇 시간 점유하는 것을 막는다. env
# CROESUS_REFRESH_BUDGET_SECONDS로 재정의(<=0 이면 예산 없음).
DEFAULT_REFRESH_BUDGET_SECONDS = 1800.0


def _refresh_budget_seconds(explicit: float | None) -> float:
    """이번 갱신에 허용할 총 시간(초). 명시값 > env > 기본(30분) 순."""
    if explicit is not None:
        return float(explicit)
    raw = os.getenv("CROESUS_REFRESH_BUDGET_SECONDS")
    if raw is None or not raw.strip():
        return DEFAULT_REFRESH_BUDGET_SECONDS
    try:
        return float(raw)
    except ValueError:
        return DEFAULT_REFRESH_BUDGET_SECONDS
...
def _run_research_refresh(
    ...
    *,
    ...
    ``deadline``(monotonic 타임스탬프)이 있으면 정성평가 LLM 루프가 예산 내에
    멈춘다 — 저하된 로컬 LLM이 락을 몇 시간 물고 있는 것을 방지한다.
    """
    ...
    if result.skipped_reason:
        ...
        return
    done = f"정성 평가 완료 — 생성 {result.generated} 실패 {result.failed}"
    if result.budget_skipped:
        done += f" (시간 예산 초과로 {result.budget_skipped}종목 미평가)"
    log(done)
...
def run_default_refresh(
    ...
    *,
    ...
    전체 실행에는 시간 예산(``budget_seconds``, 기본 30분 / env
    CROESUS_REFRESH_BUDGET_SECONDS)을 두어 저하된 LLM이 DuckDB 쓰기 락을 몇
    시간 붙들지 못하게 한다. 예산이 이미 소진됐으면 리서치 단계를 건너뛴다.
    """
    ...
    with get_write_connection(db_path) as conn:
        ...
        if include_research:
            if deadline is not None and monotonic() >= deadline:
                log("리서치 갱신 건너뜀 — 시간 예산 초과")
            else:
                try:
                    _run_research_refresh(
                        conn, log, deadline=deadline, monotonic=monotonic
                    )
